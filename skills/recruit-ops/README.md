...
**飞书招聘管家**将 OpenClaw AI 网关接入飞书，让 HR 和老板在飞书里完成整条招聘闭环，无需切换任何其他系统。
...
| **飞书日历** | 面试时间确认后自动在老板飞书日历创建日程，老板收到邀请通知 |
...
| `EXAM_PENDING` | 笔试进行中，等待候选人提交答案 |
| `EXAM_REVIEWED` | 笔试已审阅，等待决定是否进入二面 |
| `ROUND2_SCHEDULED` | 二面已安排，等候选人确认 |
...
| 笔试中 / 已发笔试 | `EXAM_PENDING` |
| 笔试完成 / 笔试已审 | `EXAM_REVIEWED` |
| 待安排二面 | `EXAM_REVIEWED` |
| 二面邀请中 / 二面已确认 | `ROUND2_SCHEDULED` |
...
LLM Agent（qwen3-max，阿里云 DashScope）
      ...
      ▼
scripts/
  ├── cmd_new_candidate.py      # HR 录入新候选人
  ├── cmd_import_candidate.py   # HR 批量导入历史候选人
  ├── cmd_round1_schedule.py    # 老板安排一面
  ├── cmd_round1_reschedule.py  # 一面改期
  ├── cmd_round1_confirm.py     # 确认一面时间 + 创建飞书日历
  ├── cmd_round1_result.py      # 一面结果（通过/拒绝）→ 触发笔试邮件
  ├── cmd_exam_result.py        # 笔试结果 → 触发二面邮件 + 飞书日历
  ├── cmd_round2_confirm.py     # 确认二面时间
  ├── cmd_round2_reschedule.py  # 二面改期
  ├── cmd_round2_result.py      # 二面结果
  ├── cmd_interview_reminder.py # 面试后催问
  ├── cmd_search.py             # 候选人模糊搜索
  ├── cmd_status.py             # 状态查询
  ├── cmd_remove.py             # 手动移除候选人
  ├── daily_exam_review.py      # 邮件扫描主程序（笔试 + 面试确认）
  ├── exam_prereview.py         # 笔试预审（代码/文本分析）
  ├── feishu_calendar.py        # 飞书日历 API（创建/删除事件 + 邀请老板）
  ├── feishu_notify.py          # 飞书消息直发（绕过 Gateway，供 cron 使用）
  ├── talent_db.py              # PostgreSQL 数据层
  └── core_state.py             # 状态机引擎 + 审计日志

外部依赖：
  ~/.openclaw/recruit-email-config.json   # IMAP/SMTP 邮箱配置
  ~/.openclaw/email-send-config.json      # SMTP 发信配置
  ~/.openclaw/talent-db-config.json       # PostgreSQL 连接配置
  ~/.openclaw/dashscope-config.json       # DashScope API Key
  ~/.openclaw/openclaw.json               # 飞书 App 配置（app_id / app_secret）
```
...
# 安排一面
python3 cmd_round1_schedule.py --talent-id t_xxxxx --time "2026-03-25 14:00" --interviewer "老板"

# 记录一面结果（通过，进入笔试）
python3 cmd_round1_result.py --talent-id t_xxxxx --result pass --email zhangsan@example.com

# 笔试结果（通过，安排二面）
python3 cmd_exam_result.py --talent-id t_xxxxx --result pass \
  --round2-time "2026-04-01 14:00" --interviewer "老板"

# 二面结果
python3 cmd_round2_result.py --talent-id t_xxxxx --result pass --notes "技术扎实，沟通流畅"

# 查询状态
python3 cmd_status.py                      # 列出所有候选人
python3 cmd_status.py --talent-id t_xxxxx  # 查单人
```
...
```bash
# 扫笔试回复（输出到终端 + 推飞书）
python3 daily_exam_review.py

# 只扫笔试（cron 模式）
python3 daily_exam_review.py --auto --exam-only

# 只扫面试确认（cron 模式）
python3 daily_exam_review.py --auto --interview-confirm-only
```

### 飞书通知测试

```bash
python3 -c "import feishu_notify as fn; fn.send_text('测试消息')"
python3 -c "import feishu_notify as fn; fn.send_text_to_hr('发给HR的消息')"
```
...
所有配置文件在 `~/.openclaw/` 目录下，**不提交到 Git**。参考 example 文件创建：

**`~/.openclaw/recruit-email-config.json`**（复制自 `config/recruit-email-config.example.json`）：
```json
{
...
**`~/.openclaw/talent-db-config.json`**（复制自 `config/talent-db-config.example.json`）：
```json
{
  "host": "localhost",
  "port": 5432,
...
**`~/.openclaw/dashscope-config.json`**（新建，不在 Git 中）：
```json
{
...
在 OpenClaw Gateway 的启动配置中设置（或直接 export）：
...
```cron
# 招聘系统 - 笔试回复扫描（每 12 小时）
0 */12 * * * /usr/bin/python3 /path/to/recruit-ops/scripts/daily_exam_review.py --auto --exam-only >> /tmp/recruit-cron.log 2>&1

# 招聘系统 - 面试确认扫描（每 8 小时）
0 */8 * * * /usr/bin/python3 /path/to/recruit-ops/scripts/daily_exam_review.py --auto --interview-confirm-only >> /tmp/recruit-cron.log 2>&1

# 招聘系统 - 面试催问（每 30 分钟）
*/30 * * * * /usr/bin/python3 /path/to/recruit-ops/scripts/cmd_interview_reminder.py >> /tmp/recruit-cron.log 2>&1
```
...
for line in sys.stdin:
    try:
        ...
        t = obj.get('time','')[:19]
        lvl = (obj.get('_meta') or {}).get('logLevelName','')
        msg = str(obj.get('0','')) + str(obj.get('1',''))
        if lvl in ('INFO','WARN','ERROR') or 'ERROR' in msg:
            print(t, '['+lvl+']', msg[:200])
    except: pass
"
...
# 只扫面试确认
python3 daily_exam_review.py --interview-confirm-only
```
...
| LLM 分析报 "未配置" | 检查 `~/.openclaw/dashscope-config.json` 中的 API Key |
| cron 扫描无输出 | 正常现象（`--auto` 模式下无新邮件时静默），查看日志确认 cron 有执行 |
| 超时通知未发送 | 检查 `daily_exam_review.py` 中的 `TIMEOUT_MINUTES` 值和时区解析 |
