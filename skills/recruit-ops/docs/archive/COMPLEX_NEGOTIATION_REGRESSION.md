...
> v3.6 起 **NEW 阶段一面派单已统一收敛到 §5.11**（HR 触发，直接进 `ROUND1_SCHEDULED + CONFIRMED`，无 PENDING 协商空间）。
> 因此本文场景在 v3.6 下仍然有效，但**触发入口**收窄为：① `WAIT_RETURN` 候选人回归后老板手动重排（仍走 §5.1）；
> ② 任何已进入 `ROUND{N}_SCHEDULED` 的候选人后续来信改期（仍走 §5.3）。NEW 阶段第一次排期不再产生本文清单里的 PENDING 协商序列。
