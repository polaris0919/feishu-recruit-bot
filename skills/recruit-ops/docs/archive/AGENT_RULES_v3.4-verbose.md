...
```
              ...
              │ inbox.cmd_analyze   （LLM 分类，stage 感知；写 ai_*）  │
              └────────────────────┬────────────────────────────────────┘
...
| **写动作必须 atomic** | 只通过 `talent.cmd_update` / `talent.cmd_delete` / `outbound.cmd_send` / `feishu.cmd_calendar_`* / `feishu.cmd_notify` 改世界。每个原子 CLI 自带自验证 + 飞书告警包装。              |
| **agent 不藏状态**   | 所有判断材料只能从：`talent_emails` 行、`talents` 行、`talent_events` 审计、`ai_payload` 中取。**禁止**把 LLM 推理结果暂存在内存或临时文件里。                                                       |
| **chain 全失败短路**  | 一个 chain 中任意一步失败：立即停止后续步骤，把错误 + 已成功步骤的 stdout 推飞书。**不试图自动回滚**——发邮件 / 删日历不可逆。                                                                                  |
...
| -------------------------------- | ----------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 「冯屹哲笔试通过，直接进 offer 阶段」           | `EXAM_SENT` / `EXAM_REVIEWED` | `POST_OFFER_FOLLOWUP` | `talent.cmd_update --stage POST_OFFER_FOLLOWUP --force --reason "boss原话: 笔试通过，直接进 offer 阶段，跳过二面"`                                                               |
...
> 偏离任何一条都会导致 CI 红灯（也就意味着自验证在生产侧会反复推飞书告警）。
> 修改 chain 必须同步改对应的测试用例。

### 5.1 安排一面（`outbound.cmd_send` → `talent.cmd_update`）

> ⚠️ **v3.6 适用范围已收窄**：NEW 阶段的一面派单**不再使用本 chain**（统一走 §5.11）。
> 本节仅保留作 **`WAIT_RETURN` 候选人回归后老板重排面试**的 chain 范式（参见 §5.8 出口
> 注释）。chain 主体未改、回归测试 `TestRound1ScheduleChain` 仍然钉死，请勿删除。


| 触发场景 | `WAIT_RETURN` 候选人回归后，老板在飞书里给候选人定了一个 round1 时间（不派单、走「等候选人 confirm 再建日历」的老路径）。 |
| ---- | ------------------------------------------------------------------------- |
...
> 模板不同（本 chain 用 `rejection_generic`，委婉口吻 + 明示"已留人才库"；
> `auto_reject` 用 `rejection_exam_no_reply`，直白口吻直说"3 天没交卷"），
...
| 退出条件 | 老板看到飞书 warn 卡片，手动二选一：①`talent.cmd_update --stage ROUND{N}_SCHEDULING --reason "candidate returned"` 让 agent 之后接 §5.1（`WAIT_RETURN → ROUND{N}_SCHEDULING` 是自然推进，**不需要** `--force`）②直接发新一轮邀请 |
...
> 📌 **v3.6 备注**：上面退出条件 ① 提到的「让 agent 之后接 §5.1」是 **§5.1 chain 章节在 v3.6 下唯一的调用点**。
> NEW 阶段一面派单已统一收敛到 §5.11；§5.1 chain 主体保留（含回归测试 `TestRound1ScheduleChain`），
> 仅服务本节出口——候选人回归后老板若直接给时间，仍走「不派单 + 等候选人 confirm + §5.2 建日历」的老路径。
...
1. **不要拼任何 chain 模拟自然流程**。例如老板说「冯屹哲笔试通过，直接进 offer 阶段」时：
...
```python
# v3.5.10：onboarding offer 的两份附件（《致邃实习协议》+《实习生入职信息登记表》）
# 由 email_templates.auto_attachments 自动追加，agent 不再手动 --attach。
# 文件被删 / 改名时 cmd_send 会立即失败拒发；维护见 §10。
...
1. **附件由系统自动追加（v3.5.10）**。模板正文里承诺了《致邃实习协议》+《实习生入职信息登记表》两份附件——`outbound.cmd_send` 走 `email_templates.auto_attachments` 自动 `--attach`，**agent 不要手动 `--attach`**。文件不在了 / 路径漂移时 `cmd_send` 会立即失败拒发（rc 非零 + 错误信息含「默认附件文件缺失」）；agent 应原样把错误回 boss + 推飞书 `severity=warn`，而不是绕开校验裸发。
...
| 触发场景 | **HR**（v3.6 起也含老板转交 HR 的等价路径）在飞书说「t_xxx 一面时间是 4-25 14:00」/「安排 张三 一面，时间 …」/同义。**前置**：候选人已经过 `intake.cmd_ingest_cv` + HR 确认入库（该 CLI 自带「人才库去重 → CV 解析 → HR 确认」交互流，**不属于 §5 chain 系列**），`current_stage=NEW`、`talents.education` 非空、`talents.has_cpp` 已写入（True/False/None 三态）。 |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| 测试源点 | `tests/test_agent_chain.py::TestRound1DispatchChain`                                                                                                                          |


**与 §5.1 的差异（必读）**：

> 📌 **v3.6 上下文**：本对比表保留作历史语义参考。v3.6 起 **NEW 阶段一面派单一律走 §5.11**，
> §5.1 chain 仅在 `WAIT_RETURN` 候选人回归后由老板手动重排时被复用（见 §5.8 出口）。
> 因此下表「触发人」一列描述的是 chain 在**各自仅剩调用场景**下的语义。


| 维度                                   | §5.1（v3.6 起仅 WAIT_RETURN 回归出口）                 | §5.11（NEW 阶段唯一 happy path）        |
| ------------------------------------ | --------------------------------------------- | --------------------------------- |
| 触发人                                  | boss（候选人回归后定时间）                                | HR（NEW 阶段录入并给时间）                  |
...
2. `**extra-attendee` 列表只能来自 `route.interviewer_open_ids**`。不允许 agent 拼任何硬编码的 open_id，包括「拍脑袋觉得这场该带上某某」。
...
6. **ambiguous / config_error 都必须 STOP**。绝对不允许兜底到「随便派一个」或「派给老板自己面」。route CLI 已经实现失败即拒（`fallback_used` 永远为 false），agent 不要绕过它。

> 📌 配置依赖：`lib.config["feishu"]["interviewer_master_open_id"]` / `interviewer_bachelor_open_id` / `interviewer_cpp_open_id` 必须配齐真实 open_id（来自 env `FEISHU_INTERVIEWER_{MASTER,BACHELOR,CPP}_OPEN_ID` 或 openclaw config 的 `interviewer{Master,Bachelor,Cpp}OpenId`）。未配齐会被哨兵值 `ou_PLACEHOLDER_*` 占位，`cmd_route_interviewer` 直接 `config_error=true`，`feishu.send_text_to_interviewer_*` / `cmd_calendar_create --extra-attendee` 也会失败即拒。
...
2. `**RECRUIT_DISABLE_SIDE_EFFECTS=1**` 环境变量下所有 sink（SMTP / IMAP / 飞书 / DB 写）都被
  `lib/side_effect_guard.py` 拦截。测试与干跑必须设置。agent 在生产环境不可设置。
...
8. `**outbound.cmd_send` 自动归一化 body（v3.5.13 / 2026-04-22）**。
  ...
  - 事故源点：冯屹哲 / t_59ej9u 2026-04-22 16:08 邮件 `f79581c1-*`，body_excerpt 里
...
10. **候选人资料目录约定（v3.5.8）**。
  - 每个候选人在 `$RECRUIT_DATA_ROOT/candidates/<talent_id>/` 下有专属目录，
   ...
   录入候选人时自动创建（告警继续：mkdir 失败不阻断录入，只飞书 warn）。
  - 三个固定子目录：
  ...
  - 测试 / 干跑：用 `RECRUIT_DATA_ROOT` 注入临时根；`RECRUIT_DISABLE_SIDE_EFFECTS=1`
  ...
  告警继续，**不影响 DB 写入成功**。
11. **一面派单约定（v3.5.7 §5.11，v3.6 收敛为唯一 happy path）**。
  - **NEW 阶段所有一面派单**（不论 HR 还是老板触发）一律走 §5.11；必须**先**调
   `intake.cmd_route_interviewer`，绝对禁止 agent 在脑子里根据 `talents.education` /
   `has_cpp` 算 open_id。
  - 三个面试官的 open_id **只能**来自 `lib.config["feishu"]["interviewer_*_open_id"]`；
  硬编码的 `ou_xxx` 字符串、占位符 `ou_PLACEHOLDER_*`、或运行时拼出的字符串
  一律禁止——`lib.feishu.send_text_to_interviewer_*` 与 `cmd_calendar_create --extra-attendee` 的占位符检测会失败即拒，但靠这层兜底很难看懂日志。
  - `cmd_route_interviewer` 输出 `ambiguous=true` 或 `config_error=true` 时**必须 STOP**，
  转 ASK_HR 分支（`feishu.cmd_notify --to hr`）。**不允许**兜底到「随便派一个」
  「派给老板自己」「派给上次的面试官」。
  - 老板若想绕开 HR 直接给 NEW 候选人定时间，agent **不要自动跑 §5.1**——通过 §3.1 表末
  「未分类的需老板介入邮件」兜底推飞书，让老板补走 HR 路径或显式 §5.9 force-jump。
  - **§5.1 chain 章节保留**，但**仅用于** `WAIT_RETURN` 候选人回归时由老板重排面试
  （见 §5.8 出口）；NEW 阶段不再调用 §5.1。两条 chain 共享 §5.3（reschedule）/
  §5.4（defer）出口，不要复制。
12. **模板默认附件（v3.5.10）**。
  - `outbound.cmd_send --template <T>` 会按 `email_templates.auto_attachments._REGISTRY`
    查表，自动追加固定附件。当前注册的：
  * `onboarding_offer` → `data/onoffer_data/模板-致邃实习协议-2026年4月版.docx`
                        + `data/onoffer_data/致邃投资-实习生入职信息登记表-2026年版.docx`
    - **agent 不要再手动 `--attach`** 这两份文件；同名文件去重，`--attach` 重复传也安全，
    但徒增噪音。
    - 文件不在了 → `cmd_send` **立即失败拒绝发送**（合同漏发是法律事故，宁可不发也不能裸发 offer）。
    - 加 / 改注册条目的流程：先把新文件提交到 `data/<sub>/`（**不能**在 `.gitignore` 里），
    再改 `_REGISTRY`，跑 `tests/test_auto_attachments.py`，最后通知老板 review 模板正文。
    - 模板正文里凡是提到「附件是 ...」必须和注册表一致；`tests/test_email_templates.py:: test_onboarding_offer_renders_all_required_vars` 同时锁定正文关键词，话术漂移会 CI 红。
