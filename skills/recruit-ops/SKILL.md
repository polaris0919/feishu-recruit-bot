...
1. 任何招聘相关消息都必须使用本 skill，包括但不限于：候选人、简历/CV、面试、一面/二面、笔试、Offer、飞书招聘 bot、候选人邮件回信、`talent_id`、`round1`/`round2`、人才库、PostgreSQL talent DB。任何邮件 sender / recipient / 正文中出现的邮箱若可能命中 `talents.candidate_email`，也先按招聘场景处理并读取本 skill。
...
6. **禁止旁路工具**：候选人邮件只能走 `outbound.cmd_send` / 对应业务 CLI（如 `exam.cmd_exam_result`、`interview.cmd_result`）。通用 `email-send` skill 不是候选人邮件入口；如果收件人 / 发件人可能是人才库候选人，先回到本 skill 做身份解析和流程选择。严禁裸 SMTP、手写邮件正文直接发给 `talents.candidate_email`；严禁用裸 SQL 改 `talents.current_stage` 或 round 字段来替代 `talent.cmd_update` / 业务 CLI。
...
1. **分类**这条消息属于哪一种：
  ...
  - **C. 写 / 动作请求** — 安排面试、记录结果、发邮件、通过飞书发 CV / 笔试答案 / 本地文件、改 stage、**运维写动作**（如 `ops.cmd_db_migrate --apply` 应用 pending migration、`ops.cmd_replay_notifications` 回放遗漏卡片）等任何会改 DB / 发外部消息的操作。
...
- **仅外部（发飞书 / 邮件 / 文件）**：`intake/cmd_send_cv.py`、`common/cmd_interview_reminder.py`、`feishu/cmd_send_file.py`、`talent/cmd_send_cv_to_feishu.py`、`exam/cmd_send_submission_to_feishu.py`
...
- HR (`feishu.hr_open_id`) is **automatically invited** to all interviews
