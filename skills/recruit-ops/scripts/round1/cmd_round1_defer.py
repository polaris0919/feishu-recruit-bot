#!/usr/bin/env python3
"""兼容入口：转发到 `interview/cmd_defer.py --round 1`。"""
import sys
...
def _send_defer_email(to_email, talent_id, candidate_name=""):
    return _REAL_SEND(to_email, talent_id, 1, candidate_name=candidate_name)


def _spawn_calendar_delete_bg(event_id):
    return _REAL_DELETE(event_id, 1)
...
def main(argv=None):
    ...
    try:
        ...
        return _mod.main(["--round", "1"] + list(args))
    finally:
