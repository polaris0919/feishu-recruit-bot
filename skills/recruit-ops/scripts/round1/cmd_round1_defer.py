...
def _send_defer_email(to_email, talent_id, candidate_name=""):
    ...
    subject = "【面试安排更新】第一轮面试暂缓，待回国后再约 - 致邃投资"
    return send_bg_email(to_email, subject, body, tag="round1_defer")


def _spawn_calendar_delete_bg(event_id):
    # type: (str) -> int
    return delete_calendar(event_id, tag="round1_defer_delete")


def parse_args(argv=None):
    p = argparse.ArgumentParser(description="暂缓一面安排，待候选人之后回国/到上海后再约")
...
def main(argv=None):
    ...
    if current_stage not in ("ROUND1_SCHEDULING", "ROUND1_SCHEDULED"):
        print(
            "ERROR: 候选人 {} 当前阶段为 {}，只有 ROUND1_SCHEDULING / ROUND1_SCHEDULED 才能暂缓一面。".format(
    ...
    old_event_id = (cand.get("round1_calendar_event_id") or "").strip()

    cand["stage"] = "WAIT_RETURN"
    cand["wait_return_round"] = 1
    cand["round1_confirm_status"] = "UNSET"
    cand["round1_time"] = None
    cand["round1_invite_sent_at"] = None
    cand["round1_calendar_event_id"] = None
    cand["round1_confirm_prompted_at"] = None
    cand["round1_reminded_at"] = None

    append_audit(
        cand,
        actor=args.actor,
        action="round1_deferred_until_return",
        payload={"reason": reason, "wait_return_round": 1},
    )
