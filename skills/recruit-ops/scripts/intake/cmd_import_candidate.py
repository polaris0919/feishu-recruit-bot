...
_STAGE_MAP = [
    # 关键词              系统阶段              round2_confirm_status  wait_return_round
    ("新候选人",          "NEW",                None,                  None),
    ("待安排一面",        "NEW",                None,                  None),
    ...
    ("笔试已审",          "EXAM_REVIEWED",      None,                  None),
    ("待安排二面",        "EXAM_REVIEWED",      None,                  None),
...
_NEXT_STEP = {
    ...
    "ROUND1_SCHEDULING":   "一面邀请已发出，系统自动扫描候选人回信；候选人确认后先通知老板/HR，需明确授权才创建日历。",
    ...
    "ROUND2_SCHEDULING":   "二面邀请已发出，系统自动扫描候选人回信；候选人确认后只通知老板，需老板再次授权才创建日历。",
...
def _map_stage(stage_text):
    # type: (str) -> tuple
    """
    将 HR 填写的中文阶段映射到 (系统阶段, round2_confirm_status, wait_return_round)。
    """
    if not stage_text:
        return None, None, None
    st = stage_text.strip()
    for kw, sys_stage, r2status, wait_return_round in _STAGE_MAP:
        if kw in st:
            return sys_stage, r2status, wait_return_round
    return None, None, None
...
def _validate(fields, stage):
    ...
    if not fields.get("stage_text"):
        ...
        errors.append(
            ...
            "笔试中 / 待安排二面 / 二面邀请中 / 二面已确认 / 待回国后一面 / 待回国后二面".format(fields.get("stage_text", ""))
        )
...
def main(argv=None):
    ...
    stage, round2_confirm_status, wait_return_round = _map_stage(fields.get("stage_text", ""))
    ...
    # EXAM_SENT/EXAM_REVIEWED 及之后：自动生成 exam_id
    exam_id = None
    needs_exam_id = stage in _NEEDS_EXAM_ID or (stage == "WAIT_RETURN" and wait_return_round == 2)
    if needs_exam_id:
    ...
    cand = {
        "talent_id":       talent_id,
        "stage":           stage,
        "wait_return_round": wait_return_round,
        "audit":           [],
        "candidate_name":  name,
    ...
    if stage == "WAIT_RETURN":
        round_label = "一面" if wait_return_round == 1 else "二面" if wait_return_round == 2 else "未记录"
        lines.append("- 暂缓轮次 : {}".format(round_label))
    if exam_id:
        lines.append("- 笔试ID   : {}".format(exam_id))
    lines.append("- DB 状态  : {}".format("已同步" if db_ok else "DB 未配置"))
    lines.append("")
    ...
    try:
        ...
        if stage == "WAIT_RETURN":
            round_label = "一面" if wait_return_round == 1 else "二面" if wait_return_round == 2 else "未记录"
            summary_lines.append("  · 暂缓轮次：{}".format(round_label))
